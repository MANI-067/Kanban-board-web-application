...
// ******************************************************************************
  ...
  getSpanBackgroundColor(dueDate:any, task:any){
    let color:string;
    const date = new Date();
    ...
    if ((dueDate.slice(0, 10)==dateString.slice(0, 10)|| dueDate.slice(0, 10)<dateString.slice(0, 10) )&& task.status!=="Completed"
    ) {
      return 'warning';
    } 
    return '';
